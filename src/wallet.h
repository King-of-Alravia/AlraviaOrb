...
#ifndef ALRAVIAORB_WALLET_H
#define ALRAVIAORB_WALLET_H
...
{
private:
    bool SelectOrbs(int64 nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setOrbsRet, int64& nValueRet) const;
...
public:
    ...
    std::set<COutPoint> setLockedOrbs;

    // check whether we are allowed to upgrade (or already support) to the named feature
    bool CanSupportFeature(enum WalletFeature wf) { return nWalletMaxVersion >= wf; }

    void AvailableOrbs(std::vector<COutput>& vOrbs, bool fOnlyConfirmed=true) const;
    bool SelectOrbsMinConf(int64 nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vOrbs, std::set<std::pair<const CWalletTx*,unsigned int> >& setOrbsRet, int64& nValueRet) const;
    bool IsLockedOrb(uint256 hash, unsigned int n) const;
    void LockOrb(COutPoint& output);
    void UnlockOrb(COutPoint& output);
    void UnlockAllOrbs();
    void ListLockedOrbs(std::vector<COutPoint>& vOutpts);
...
public:
    ...
    {
        // Must wait until Orbbase is safely deep enough in the chain before valuing it
        if (IsOrbBase() && GetBlocksToMaturity() > 0)
            return 0;
    ...
    {
        if (IsOrbBase() && GetBlocksToMaturity() > 0 && IsInMainChain())
        {
