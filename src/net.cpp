...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("alraviaorb-ext-ip");
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "AlraviaOrb " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"alraviaorb.sipa.be", "seed.alraviaorb.sipa.be"},
    {"bluematt.me", "dnsseed.bluematt.me"},
    {"dashjr.org", "dnsseed.alraviaorb.dashjr.org"},
    {"xf2.org", "bitseed.xf2.org"},
    {NULL, NULL}
};

static const char *strTestNetDNSSeed[][2] = {
    {"alraviaorb.petertodd.org", "testnet-seed.alraviaorb.petertodd.org"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. AlraviaOrb client is probably already running."), addrBind.ToString().c_str());
        else
...
{
    printf("StopNode()\n");
    GenerateAlraviaorbs(false, NULL);
    MapPort(false);
